...
- **A write that did not happen no longer answers like one that did.** `mempalace_kg_invalidate` matches `object` verbatim against stored facts, and its `UPDATE ... WHERE valid_to IS NULL` silently touches no rows when nothing matches — a typo, a re-worded fact, or one that was already ended. The response was `{success: true, fact, ended}` regardless, and that `fact` string is the caller's own input reassembled, never a stored row, so it read as confirmation of a retirement that never occurred and the stale fact stayed live and current in every later query. `KnowledgeGraph.invalidate()` now returns the number of open facts it closed, and the tool returns that as `matched`; a call that closed nothing returns `success: false` with `matched: 0` and `error_class: "NoMatchingFact"` rather than an additive field an existing caller would keep ignoring — the whole failure mode was a caller reading `success` and believing it. The Hermes provider's `mempalace_kg_invalidate` follows the same contract. A caller that retries an already-ended fact now gets told it closed nothing, which is the honest answer rather than the previous silent second "success".
- **Refused writes now leave a trace in `mcp_server.log`.** When a peer process held the palace writer lease, every mutating tool was turned away with a structured error to the MCP client and *nothing at all* in the log — while the sibling `except Exception` branches beside those handlers logged theirs, so the common failure was the silent one and a whole session of refused writes was diagnosable only by guesswork. The dispatch-level peer-writer refusal now logs the tool name, the holder reason and the failure kind, and the `MineAlreadyRunning` handlers in `mempalace_mine`, `mempalace_sync` and `mempalace_diary_write` route through one `_refused_write()` helper that logs as it builds the refusal payload. The payload shape is unchanged, including the `error_class` the daemon keys its defer-vs-dead-letter decision on (#2014).
